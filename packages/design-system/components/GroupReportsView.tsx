...
>(function DateSelectorTrigger({ label, displayValue, icon, onClick }, ref) {
  return (
    <button
      ...
      ref={ref}
      onClick={onClick}
      className="flex-1 bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] px-[15px] py-[10px] flex items-center justify-between relative cursor-pointer hover:bg-gray-50 transition-colors h-[56px] min-w-0 w-full"
    >
...
const FigmaSelectFilter = ({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-[5px] min-w-0 flex-1 pointer-events-none">
        <div className="text-black shrink-0">{icon}</div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base font-semibold text-[#0097B2]">
                {formatSecondsToTime(groupSummary.avgSeconds)}
              </p>
            </div>
          </div>
          <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] p-3 flex items-center gap-3">
...
export function GroupReportsView({
  ...
  const loadData = async () => {
    try {
      ...
      if (result && result.length > 0) {
        const extractedOptions = extractFilterOptionsFromMetrics(result);
        setFilterOptions((prev) => {
          ...
          return {
            ...prev,
            teams: extractedOptions.teams,
            jobPositions: extractedOptions.jobPositions,
          };
        });
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (teamId && clientId) {
      const availableTeams = filterOptions?.teams?.filter((t) => t.parentValue === clientId) || [];
      const currentTeam = availableTeams.find((t) => t.value === teamId);
      if (!currentTeam && availableTeams.length > 0) {
        setTeamId("");
      }
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const avgProductivity = Math.round(
      metrics.reduce((acc, m) => acc + m.productivity_score, 0) / totalContractors,
    );
  ...
  const groupedMetrics = useMemo(() => {
      ...
      return [
        {
          ...
          contractors: filteredMetrics.filter(
            (m) => m.client_id === clientId && m.team_id === teamId,
          ),
        },
      ];
    }
    ...
    if (clientId) {
      ...
      const teamsMap = new Map<
        string,
        { teamId: string; teamName: string; contractors: RealtimeMetrics[] }
      >();
      ...
      const clientName =
        filterOptions?.clients.find((c) => c.value === clientId)?.label || "Unknown";
  ...
  return (
    <div
      className="p-2 md:p-8 min-h-screen bg-[#FFFFFF] w-full"
      style={{ overflowX: "hidden", boxSizing: "border-box" }}
    >
      <div
        className="max-w-full flex flex-col gap-4 md:gap-6 w-full"
      ...
      >
        ...
        <div className="flex items-center justify-between w-full max-w-full min-w-0 overflow-hidden">
          <div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1 overflow-hidden">
            {onBack && (
              <button
                onClick={onBack}
                className="p-1 hover:bg-gray-100 rounded-full transition-colors text-black shrink-0 cursor-pointer"
              >
                <ArrowLeft className="w-6 h-6" />
              </button>
            )}
            <h1 className="text-xl md:text-2xl font-semibold text-black truncate min-w-0">
              {t("previewReport")}
            </h1>
          </div>
          <Link href={exportUrl}>
        ...
        {/* Mobile Layout */}
        <div
          className="flex flex-col lg:hidden gap-1 md:gap-5 w-full min-w-0"
            ...
            <>
                ...
                <div className="flex flex-col gap-[10px] w-full">
                  <div className="flex gap-[10px] w-full">
                    <div className="bg-white border border-[rgba(166,166,166,0.25)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px] h-[63px]">
                      ...
                      <div className="flex flex-col items-start leading-none min-w-0">
                        <p className="text-[12px] font-light text-black mb-1">
                          {t("totalContractors")}
                        </p>
                        <p className="text-[20px] font-semibold text-black">
                          {summaryMetrics?.totalContractors || 0}
                        </p>
                      </div>
                    </div>
                    <div className="bg-white border border-[rgba(166,166,166,0.25)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px] h-[63px]">
                      ...
                      <div className="flex flex-col items-start leading-none min-w-0">
                        ...
                        <p className="text-[20px] font-semibold text-[#0097B2]">
                          {summaryMetrics?.avgProductivity || 0}%
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-[10px] w-full">
                    <div className="bg-white border border-[rgba(166,166,166,0.25)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px] h-[63px]">
                      ...
                      <div className="flex flex-col items-start leading-none min-w-0">
                        <p className="text-[12px] font-light text-black mb-1">
                          {t("totalKeyboardInputs")}
                        </p>
                        <p className="text-[20px] font-semibold text-black">
                          {summaryMetrics?.totalKeyboard.toLocaleString() || 0}
                        </p>
                      </div>
                    </div>
                    <div className="bg-white border border-[rgba(166,166,166,0.25)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px] h-[63px]">
                      ...
                      <div className="flex flex-col items-start leading-none min-w-0">
                        ...
                        <p className="text-[20px] font-semibold text-black">
                          {summaryMetrics?.totalMouse.toLocaleString() || 0}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex gap-[10px] w-full">
                <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px] h-[68px]">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    <p className="text-[9px] font-light text-[#0097B2] leading-none mb-1">
                      {t("mostActiveContractor")}
                    </p>
                    <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                      <p className="text-[11px] font-semibold text-black truncate min-w-0">
                        {summaryMetrics?.mostActive?.contractor_name || "N/A"}
                      </p>
                      {summaryMetrics?.mostActive?.country &&
                        getCountryCode(summaryMetrics.mostActive.country) && (
                          <ReactCountryFlag
                ...
                <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px] h-[68px]">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    <p className="text-[9px] font-light text-[#FF0004] leading-none mb-1">
                      {t("leastActiveContractor")}
                    </p>
                    <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                      <p className="text-[11px] font-semibold text-black truncate min-w-0">
                        {summaryMetrics?.leastActive?.contractor_name || "N/A"}
                      </p>
                      {summaryMetrics?.leastActive?.country &&
                        getCountryCode(summaryMetrics.leastActive.country) && (
                          <ReactCountryFlag
                      ...
                      <div className="bg-[rgba(255,0,4,0.25)] px-[10px] py-[3px] rounded-[5px] flex items-center justify-center shrink-0">
                        <span className="text-[9px] font-medium text-[#FF0004] leading-none">
                          {Math.round(summaryMetrics?.leastActive?.productivity_score || 0)}%
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] px-[20px] py-[28px] flex flex-col gap-[15px] w-full">
                ...
                <div className="flex items-center justify-between w-full gap-[5px]">
                  <div className="flex flex-col items-center min-w-0 flex-1">
                    <p className="text-[20px] font-semibold text-black">
                      {formatSecondsToTime(summaryMetrics?.totalSeconds || 0)}
                    </p>
                  ...
                  <div className="flex flex-col items-center min-w-0 flex-1">
                    ...
                    <p className="text-[20px] font-semibold text-black">
                      {formatSecondsToTime(summaryMetrics?.avgActiveSeconds || 0)}
                    </p>
                  </div>
                  <div className="flex flex-col items-center min-w-0 flex-1">
                    <div className="flex items-center gap-[5px] mb-1">
                    ...
                    <p className="text-[20px] font-semibold text-black">
                      {formatSecondsToTime(summaryMetrics?.avgIdleSeconds || 0)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {groupedMetrics.length > 0 ? (
                groupedMetrics.map((group, groupIndex) => (
                  <div
                    key={`${group.clientId}-${group.teamId}-${groupIndex}`}
                    className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] px-[20px] py-[28px] w-full"
                  >
            ...
            <>
              ...
              <div className="flex flex-col lg:flex-row gap-5">
                ...
                <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] px-[17px] py-[28px] flex flex-col gap-[10px] w-full lg:w-[45%]">
                  <h3 className="text-[20px] font-semibold text-black leading-none mb-2">
                    {t("summaryMetrics")}
                  </h3>
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px]">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-light text-[#0097B2] leading-none mb-1">
                          {t("mostActiveContractor")}
                        </p>
                    ...
                    <div className="bg-white border border-[rgba(166,166,166,0.5)] rounded-[5px] p-[10px] flex-1 flex items-center gap-[10px]">
                      <ChartNoAxesColumnDecreasing className="w-[30px] h-[30px] text-[#FF0004] shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-light text-[#FF0004] leading-none mb-1">
                          {t("leastActiveContractor")}
                        </p>
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-[14px] font-semibold text-black truncate">
                  ...
                  groupedMetrics.map((group, groupIndex) => {
                    ...
                    if (contractors.length > 0) {
                      const totalSeconds = contractors.reduce(
                        (acc, m) => acc + m.total_session_time_seconds,
                        0,
                      );
                      const avgSeconds = totalSeconds / contractors.length;
                      const avgProductivity = Math.round(
                        contractors.reduce((acc, m) => acc + m.productivity_score, 0) /
                          contractors.length,
                      );
                      groupSummary = {
