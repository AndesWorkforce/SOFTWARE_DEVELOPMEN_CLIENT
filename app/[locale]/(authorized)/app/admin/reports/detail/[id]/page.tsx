...
export default function ReportDetailPage() {
  ...
  const handleStartDateChange = useCallback(
    (newStartDate: string) => {
      ...
      router.replace(
        `/${locale}/app/admin/reports/detail/${contractorId}?from=${newStartDate}&to=${endDate}`,
        { scroll: false },
      );
    },
    [locale, contractorId, endDate, router],
  );
  ...
  const handleEndDateChange = useCallback(
    (newEndDate: string) => {
      ...
      // Actualizar URL sin scroll para mantener la posición
      router.replace(
        `/${locale}/app/admin/reports/detail/${contractorId}?from=${startDate}&to=${newEndDate}`,
        { scroll: false },
      );
    },
