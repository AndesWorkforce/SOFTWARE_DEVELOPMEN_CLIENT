...
type AddJobPositionFormValues = {
  name: string;
  description: string;
};

export function AddJobPositionModal({ onClose }: AddJobPositionModalProps) {
  const t = useTranslations("jobPositions.modal");
  const tCommon = useTranslations();
  const [isPending, startTransition] = useTransition();
  const [showConfirm, setShowConfirm] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [pendingPayload, setPendingPayload] = useState<AddJobPositionFormValues | null>(null);
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<AddJobPositionFormValues>({
    resolver: zodResolver(schema),
    defaultValues: { name: "", description: "" },
  });

  const onSubmit = (data: AddJobPositionFormValues) => {
  ...
  const handleConfirmAdd = () => {
    ...
    startTransition(() => {
      try {
        jobPositionsService.create({
          name: pendingPayload.name,
          description: pendingPayload.description || undefined,
        });
        ...
        const message =
          error instanceof Error
            ? error.message
            : t("errorCreating") || "Error creating job position.";
  ...
  return (
    <>
      <FormModalLayout
        ...
        onClose={onClose}
        title={t("addTitle") || "Add Job Position"}
        size="md"
        contentPadding="30px 40px"
        modalStyle={{ width: "100%", maxWidth: "583px" }}
      >
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-[503px] mx-auto">
          <div className="flex flex-col gap-[25px] items-start w-full">
            ...
            <div className="flex flex-col md:flex-row gap-[10px] w-full pt-2">
              ...
              >
                {isPending ? t("adding") || "Adding..." : t("addButton") || "Add Job Position"}
              </Button>
              <Button
                type="button"
                onClick={onClose}
            ...
            <div className="flex flex-col gap-[30px] items-center w-full max-w-[343px] md:max-w-[400px]">
              <div className="flex flex-col gap-[15px] text-center w-full">
                <h2 className="text-[24px] font-bold text-black">
                  {t("confirmTitle") || "Add Job Position?"}
                </h2>
                <p className="text-[16px] font-normal text-[#1E1E1E]">
                  {t("confirmSubtitle") || "Are you sure you want to add this job position?"}
                </p>
              </div>
            ...
            <div className="flex flex-col items-center justify-between h-[184px] w-[360px] gap-[15px]">
              <div className="flex flex-col items-center gap-[15px] w-full">
                ...
                <p className="text-[16px] text-center text-[#1e1e1e] font-normal leading-normal">
                  {t("successTitle") || "Job position added successfully"}
                </p>
              </div>
              <Button
                type="button"
