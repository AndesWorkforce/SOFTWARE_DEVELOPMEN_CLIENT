...
export function DeleteJobPositionModal({
  ...
  const handleConfirmDelete = () => {
    ...
    setError(null);
    try {
      jobPositionsService.remove(positionId);
      if (onDeleted) onDeleted();
      setShowSuccess(true);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : t("errorDeleting") || "Error deleting job position.",
      );
    } finally {
      setLoading(false);
    }
              ...
              <div className="flex flex-col gap-[15px] text-center w-full">
                ...
                <p className="text-[16px] font-normal text-[#1E1E1E]">
                  {t("deleteConfirmSubtitle") ||
                    "This action cannot be undone. The job position will be permanently removed."}
                </p>
              </div>
              {error && <p className="text-[#FF0004] text-sm w-full text-center">{error}</p>}
            ...
            <div className="flex flex-col items-center justify-between h-[184px] w-[360px] gap-[15px]">
              <div className="flex flex-col items-center gap-[15px] w-full">
                ...
                <p className="text-[16px] text-center text-[#1e1e1e] font-normal leading-normal">
                  {t("deleteSuccessTitle") || "Job position deleted successfully"}
                </p>
              </div>
              <Button
                type="button"
