...
export function AddClientModal({ onClose }: AddClientModalProps) {
  ...
  const [pendingPayload, setPendingPayload] = useState<AddClientFormValues | null>(null);
  ...
  // Al enviar el formulario, primero mostramos un modal de confirmación (igual que contractors)
  ...
  const handleConfirmAdd = async () => {
    ...
    startTransition(() => {
      (async () => {
        try {
          const name = pendingPayload.name.trim();
          const email = pendingPayload.email.trim();
          const password = pendingPayload.password.trim();
          ...
          const message =
            error instanceof Error
              ? error.message
              : t("errorCreating") || "Error creating client. Please try again.";
          ...
          <div className="flex flex-col gap-[25px] items-start w-full">
            ...
            >
              <Input
                type="password"
                {...register("password")}
                placeholder={t("passwordPlaceholder") || "Type password here..."}
                className={FORM_CONTROL_CLASS}
